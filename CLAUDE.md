...
- **Documentation**: Antora site lives in `docs/`. Playbook is `docs/antora-playbook.yml`. Build with `./docs/build.sh`. After build, docs are served at `/docs/` by the Fastify server (registered as a second `@fastify/static` plugin in `server.ts`). Supplemental UI overrides are in `docs/supplemental-ui/partials/`. Uses Antora 2.x — no extensions, attributes go in the playbook, version must be explicit (not `~`). Branding config (including the Documentation link) is in `packages/agent-frontend/public/branding/product.json`.
